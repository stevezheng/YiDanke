...
PCAS.SetP = function (a) {
  for (i = 0; i < PCAP.length; i++) {
    PCAPT = PCAPV = PCAP[i];
    if (PCAPT == SPT) {
      PCAPV = ""
    }
    a.SelP.options.add(new Option(PCAPT, PCAPV));
    if (a.DefP == PCAPV) {
      a.SelP[i].selected = true
    }
  }
  PCAS.SetC(a)
};
PCAS.SetC = function (a) {
  PI = a.SelP.selectedIndex;
  a.SelC.length = 0;
  for (i = 1; i < PCAC[PI].length; i++) {
    ...
    a.SelC.options.add(new Option(PCACT, PCACV));
    if (a.DefC == PCACV) {
      a.SelC[i - 1].selected = true
    }
  }
  if (a.SelA) {
    PCAS.SetA(a)
...
PCAS.SetA = function (a) {
  PI = a.SelP.selectedIndex;
  CI = a.SelC.selectedIndex;
  a.SelA.length = 0;
  for (i = 1; i < PCAA[PI][CI].length; i++) {
    ...
    a.SelA.options.add(new Option(PCAAT, PCAAV));
    if (a.DefA == PCAAV) {
      a.SelA[i - 1].selected = true
    }
  }
};